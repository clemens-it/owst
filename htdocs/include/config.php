...
//the rights to access the directory of the web script. therefore the 'cd /'
$cfg['at_cmd_line'] = "cd /; echo $cfg_executable | at ";

//atq command line
$cfg['atq_cmd_line'] = "(atq | sort -k6n -k3M -k4n -k5n) 2>&1";

//empty at queue command line
$cfg['atempty_cmd_line'] = "(atq | cut -f1 | xargs --no-run-if-empty atrm) 2>&1";


//at user - user which owns the AT queue
//a corresponding line in /etc/sudoers is required for www-data to show the AT queue:
// www-data ALL = (owst) NOPASSWD: /usr/bin/atq, /usr/bin/atrm, /usr/bin/at
//only required if using sudo model
//$cfg['at_user'] = 'owst';

//at_cmd_line using sudo model
//$cfg['at_cmd_line'] = "cd /; echo $cfg_executable | sudo -nu ".escapeshellarg($cfg['at_user']) ." at ";

//atq_cmd_line using sudo model
//$cfg['atq_cmd_line'] = "(sudo -nu ".escapeshellarg($cfg['at_user']) ." atq | sort -k6n -k3M -k4n -k5n) 2>&1";

//atempty_cmd_line using sudo model
