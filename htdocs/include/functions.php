...
function checkOWType($addr, $type, $info)
{
    ...
    if ($ow_type == null) {
        logEvent(
            "OW device $addr is not present, or {$addr}type does not exist. "
            . "Program id: {$info['tpid']}, switch id: {$info['sid']}",
            LLWARN
        );
        return false;
    }
...
{
    //in case a switch is in mode 'timer' there could be the following scenario:
    //its mode was previously 'on' or 'off'. When switching back to 'timer' we
    //need to decide how to set the OW state depending on whether there are any
    //time programs with active=1 or active=0 respectively.  This functions sets
    //a corresponding state into the new_ow_state-array, which can be overridden
    //in case a corresponding time program is about to change at the same call.
    //This part of modechange (s.mode = timer) must be processed before cycling
    //through the time programs.  It checks all the switches which are in mode
    //'timer' and determines and sets a new state according to the time programs

    logEvent('ModeChangeTimer Checking for active time_programs', LLDEBUG);

    //query explanation: "LEFT OUTER JOIN" to get all the switches. "now BETWEEN
    //valid_from AND valid_until" to get only valid time program this however
    //makes the LEFT OUTER JOIN ineffective therefore the "OR tp.id IS NULL". In
    //case a switch has no time programs at all, it will be shown, in case a
    //switch has only invalid time programs (i.e. all laying in the past and/or
    //future) it will not be shown. Therefore the effort with the subquery and
    //reapplied left outer join
    $sql = "
    ...
    if ($sth === false) {
        logEvent(
            ...
            . implode('; ', $dbh->errorInfo()) ."; Terminating.",
            LLERROR
        );
    ...
    while ($result = $sth->fetch(PDO::FETCH_ASSOC)) {
        ...
        //values for sumactive may be: NULL, 0, any number > 0. The first two
        //will be converted to 0, the last to 1
        $new_ow_state[$ow_path] = ($result['sumactive'] >= 1 ? 1 : 0);
        $switch_mode[$ow_path] = $result['mode'];
        logEvent(
            "Modechange: Switch $ow_path in timer mode. Time programs suggest new state: {$new_ow_state[$ow_path]}",
            LLINFO
        );
    }
    $sth->closeCursor();
...
{
    //Saves the mode for all switches in non-timer mode and create also a
    //corresponding entry in array $new_ow_state as the next code block below
    //will cycle through $new_ow_state
    //All the due time programs need to be cycled through anyway, because they
    //need to have their AT jobs reprogrammed.  This part of modechange needs to
    //be after cycling through the due time programs, in order to be able to
    //overwrite $new_ow_state
    ...
    while ($result = $sth->fetch(PDO::FETCH_ASSOC)) {
        ...
        //with switch_mode limited to 'on' or 'off' due to the sql query any
        //value will do for new_ow_state
...
function emptyAtQueue()
{
    ...
    if ($retval == 0) {
        ...
        logEvent(
            "Emptying of at queue failed. Return value: $retval. Output: ".implode("--", $output),
            LLERROR
        );
    }
    return ($retval == 0);
...
function reprogramAt($dbh, $limit_to_tpid="")
{
    ...
    $sql = "
		...
		FROM time_program tp
			INNER JOIN switch s ON tp.switch_id = s.id
    ...
    while ($result = $sth->fetch(PDO::FETCH_ASSOC)) {
        ...
        //depending on whether a job is active or not we need to consider either
        //switch_off_time or switch_on_time
        if (!$result['active']) {
...
function handleTimePrograms($dbh, &$new_ow_state, &$switch_mode, &$time_programs_to_delete, $reprogramming_at)
{
    ...
    $wday = date('w');
    $sql = "
		SELECT 'on' AS qry, tp.id AS tpid, s.id AS sid, *, -1 AS time_beeing_active
    ...
    while ($result = $sth->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($result['qry'] == 'on') {
            ...
            if ($result['switch_off_priority'] == 'runtime') {
                ...
                logEvent(
                    "Reprogramming (prio time off) to switch off at {$result['switch_off_time']} on $date_i."
                    . " Time program id {$result['tpid']}",
                    LLINFO_ACTION
                );
        ...
        if ($result['qry'] == 'off') {
            ...
            if ($result['override_other_programs_when_turning_off']) {
                ...
                $sql = "
					SELECT COUNT(*) AS ct
					FROM time_program
					WHERE id != {$result['tpid']}
						AND active
						AND switch_id = {$result['switch_id']}
				";
                ...
                logEvent(
                    "Checking whether there are any other active programs for path $ow_path: {$ct['ct']} active.",
                    LLDEBUG
                );
                $sth_ovr->closeCursor();
                $sth_ovr = null;
            ...
            logEvent(
                "Updated time_program to inactive. Id {$result['tpid']}, Rows affected by query: $ra",
                LLINFO_ACTION
            );
            ...
            if ($found) {
                if (!$reprogramming_at) {
                    logEvent(
                        "Reprogramming to switch on at {$result['switch_on_time']} on $date_i."
                        . " Time program id {$result['tpid']}",
                        LLINFO_ACTION
                    );
                       //reprog at
...
function setOWSwitch($switch_mode, $new_ow_state)
{
    ...
    foreach ($new_ow_state as $k => $v) {
        ...
        logEvent(
            "Switch is in mode {$switch_mode[$k]}. Current state: $ow_state. Setting new OW state for $k: $v",
            LLINFO_OWACTION
        );
        if ($ow_state != $v) {
            put($k, $v);
...
function deleteExpiredTimePrograms($dbh, $time_programs_to_delete)
{
    ...
    $sql = "
		DELETE FROM time_program
		WHERE id IN (".implode(', ', array_keys($time_programs_to_delete)) .")
    ...
    logEvent(
        "Deleted time programs with IDs ("
        . implode(', ', array_keys($time_programs_to_delete)) ."). Rows affected by query: $ra",
        LLINFO_ACTION
    );
    return true;
